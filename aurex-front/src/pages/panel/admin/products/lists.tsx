import { Product, ProductStatus, Stock, VolumeType } from "../../../../interfaces/Product";
import { Eye, EyeOff, Pencil, Trash2, Download, FileSpreadsheet, Upload } from "lucide-react";
...
export default function ProductsListPage() {
    ...
    reader.onload = (e) => {
      ...
      if (data) {
        try {
          ...
          setImportError(`Error al procesar el archivo: ${(error as Error).message}`);
        }
      }
      setImportLoading(false);
    };
  ...
  const formatProducts = (data: any[]): Product[] => {
    ...
    rows.forEach((row, index) => {
      try {
        ...
        setImportError(`Error en fila ${index + 2}: ${(error as Error).message}`);
      }
    });
  ...
  const handleCloseSummary = () => {
    ...
    if (successCount > 0) {
      handleCloseImport();
      products.get(); // Recargar la lista de productos
    }
  };
                  ...
                  <>
                    {importError && (
                      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                        ...
                        <p className="text-yellow-600 text-sm mt-2">{importError}</p>
                      </div>
                    )}
